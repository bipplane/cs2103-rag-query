...
def home():
    ...
    <html>
    <head>
        <title>CS2103 Ask Me Anything</title>
        <style>
            body { font-family: Arial, sans-serif; max-width: 800px; margin: 50px auto; padding: 20px; }
            h1 { color: #333; }
            #query { width: 100%; padding: 12px; font-size: 16px; border: 2px solid #ddd; border-radius: 8px; }
            #submit { margin-top: 20px; padding: 12px 24px; font-size: 16px; background: #007bff; color: white; border: none; border-radius: 8px; cursor: pointer; }
            #submit:hover { background: #0056b3; }
            #submit:disabled { background: #ccc; cursor: not-allowed; }
            #cooldown { margin-left: 10px; color: #666; font-size: 14px; }
    ...
    <body>
        ...
        <script>
            ...
            function updateCooldownDisplay() {
                ...
                const remaining = COOLDOWN_MS - timeSinceLastAsk;
            ...
            async function askQuestion() {
                ...
                const now = Date.now();
                const timeSinceLastAsk = now - lastAskTime;
                if (timeSinceLastAsk < COOLDOWN_MS) {
                    return;
                }
                lastAskTime = now;
